...
(para problemas de domínio discreto) ou de números reais (para domínios 
contínuos) \citep{Luke2013Metaheuristics}. No entanto, estas representações não 
são adequadas para a \ac{tmap}.
...
mesmo centro (o que não quer dizer partições iguais). Além disso, se as 
partições são construídas a partir de um determinado centro, distanciar esses 
centros ajudaria a deixar as partições o mais disjuntas possível.
...
dois patrulham o grafo. Cada um ficou sob responsabilidade de uma partição, onde 
irão percorrer um ciclo indefinidamente. O agente vermelho, ficou com o centro 
$5$, enquanto que o verde com o centro 3.
...
para a construção de indivíduos, a mutação e a recombinação.
...
aleatoriedade que considerado importante nas inicializações de indivíduos nos 
...
Sampaio\footnote{\url{http://lattes.cnpq.br/8865836949700771}}.
 ...
 A \tabref{tbl:resumo_operadores} resume os operadores propostos, indicando a 
...
No próximo capítulo, será avaliada a efetividade desses operadores, através dos 
resultados de simulações e comparação com outras abordagens propostas por 
outros autores.