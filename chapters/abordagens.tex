...
\citep{nilsson2003heuristics}. Como o objetivo do operador de mutação é 
...
Já que o \textit{2-change} é uma heurística para o \ac{tsp}, ele será explicado 
...
\begin{multline*}
\centering
\langle V_{1}, ..., V_{e_{1}}, V_{e_{1}+1}, ..., V_{e_{2}}, V_{e_{2}+1}, ..., V_{|P|} \rangle
\end{multline*}
...
\begin{multline*}
\centering
$$ \langle V_{1}, ..., V_{e_{1}} \rangle \oplus P_{e_{2}, e_{1}+1} \oplus \langle V_{e_{2}+1}, ..., V_{|P|} \rangle $$
\end{multline*}

Onde $P_{e_{2}, e_{1}+1}$ é o subcaminho de $P$, que vai de $e_{1}+1$ para 
$e_{2}$, invertido e $\oplus$ representa uma operação de concatenação. Para o 
caso dos grafos utilizados nessa pesquisa (não necessariamente completos), 
utilizou-se este resultado, mas ao invés de montar o caminho com a aresta que 
liga quaisquer dois vértices consecutivos em $P^{\prime}$, utilizou-se sempre 
...
(de acordo com a classificação apresentada na \secref{sec:classifytmap}) como 
